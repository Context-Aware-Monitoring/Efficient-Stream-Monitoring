...
class RewardInformation:
    ...
    def __init__(self, reward_filepath):
        self._reward_df = pd.read_csv(reward_filepath)
    ...
    def get_top_rank_count_for_arms(self):
        ...
        print('A total of %d different arms atleast once receive the highest reward' % len(list(filter(lambda x : x > 0, list(arm_to_count.values())))))
        return sorted(list(arm_to_count.items()), reverse=True, key=lambda x : x[1])
    ...
    def get_top_L_rank_count_for_arms(self, L):
        ...
        print('A total of %d different arms atleast once receive a reward within the top-%d' % (len(list(filter(lambda x : x > 0, list(arm_to_count.values())))), L))
        return sorted(list(arm_to_count.items()), reverse=True, key=lambda x : x[1])
    ...
    def get_average_reward(self):
        ...
        return sorted(list(arms_to_avg_reward.items()), reverse=True, key=lambda x: x[1])
